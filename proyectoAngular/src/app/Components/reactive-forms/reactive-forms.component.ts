...
const API_END_POINT: string = "https://curso-e421f.firebaseio.com/Employee";
  ...
  loadAllUsers(): void {
    this.http.get(API_END_POINT + '.json').subscribe(
      ...
      response => {
        ...
        for (let key in data) {
          this.users.push(new Employee(data[key].name, data[key].email, data[key].job, data[key].age, data[key].isActive, data[key].valoration, key));
        }
      },
      ...
      //Callback final
      () => {
        console.log("Fin de la petición loadAllUsers()");
      }
    );
  }
